        ...
        try
        {
            ...
            if (action == "withdrawPP")
            {
                ...
                if (localFunds.ActiveInUserAccount >= funds.AmountToWithdraw)
                {
                    ...
                    //PaymentManagerV2 pg = new PaymentManagerV2();

                    //var f = pg.StartInvoiceWizard()
                    //    .Initalize(RollinNewsConfig.MERCHANT_ID, "USD", PaymentProvider.Paypal, Convert.ToBoolean(ConfigurationManager.AppSettings["IsPayPalLive"].ToString()), RDN.Library.Classes.Payment.Enums.ChargeTypeEnum.RollinNewsWriterPrePayout)
