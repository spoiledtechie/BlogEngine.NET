...
public partial class _default : BlogEngine.Core.Web.Controls.BlogBasePage
{
    ...
    private void DisplayAuthorPosts()
    {
        if (!string.IsNullOrEmpty(Request.QueryString["name"]))
        {
            ...
            if (Int32.TryParse(author, out authorId))
            {
                UsersRepository rep = new UsersRepository();
                var auth = rep.GetUser(authorId);

                var profile = AuthorProfile.GetProfile(auth.UserName);
                author = profile.DisplayName;
                userName = auth.UserName;
            }

            PostList1.ContentBy = ServingContentBy.Author;
            PostList1.Posts = Post.GetPostsByAuthor(userName).ConvertAll(new Converter<Post, IPublishable>(delegate (Post p) { return p as IPublishable; }));
