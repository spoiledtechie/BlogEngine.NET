...
using BlogEngine.Core.Funds.Monthly;
...
namespace BlogEngine.Core.Funds
{
    public class MonthlyStatementsFactory
    {
        ...
        private List<Post> PostsToPay { get; set; }
        private List<Post> PostsForAll { get; set; }
        ...
        public MonthlyStatementsFactory GetPostsThatHadViews()
        {
            ...
            PostsToPay = (from xx in dc.PostsDb
        ...
        public MonthlyStatementsFactory ZeroOutMonthlyViews()
        {
            ...
            {
                var post = dc.PostsDb.Where(x => x.PostId == p.Id).FirstOrDefault();
                if (post != null)
                {
        ...
        public MonthlyStatementsFactory SetAuthorsForPosts(List<Post> posts)
        {
            PostsForAll = posts;
        ...
        //    var allPosts = PostsToPay.Where(x => x.DisablePaymentsForPost == false).Where(x => x.AuthorUserId == LibraryConfig.DEFAULT_SCOTTS_USER_ID || x.AuthorUserId == LibraryConfig.DEFAULT_ADMIN_USER_ID).ToList();
        ...
        //        };
        //            if (!String.IsNullOrEmpty(member.SiteName))
        //                emailData.Add("name", member.SiteName);
        //            else
        //                emailData.Add("name", member.SiteName);
